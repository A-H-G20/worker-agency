...
if (isset($_GET['id'])) {
    ...
    try {
        ...
        header("Location: admin_management.php?message=User%20and%20related%20records%20deleted%20successfully");
        exit();
    } catch (Exception $e) {
        ...
        header("Location: admin_management.php?error=An%20error%20occurred:%20" . urlencode($e->getMessage()));
        exit();
    }
} else {
    // Redirect to the user management page with an error message if no ID is set
    header("Location: admin_management.php?error=No%20user%20ID%20provided");
    exit();
}
